...
Copy-Item -Path \\$env:userdnsdomain\SYSVOL\global.sletat.ru\soft\ShareX -Destination $env:PUBLIC\temp\ -Recurse -Force

#Check ShareX installation
$ShareX=(Get-CimInstance -Class Win32_Product | Where-Object Name -eq "ShareX")
$instaled=(Test-Path $env:PUBLIC\installed.log)

if ($null -eq $ShareX){
    #Set local path
    Set-Location $env:SystemRoot\temp\ShareX
    ...
    Copy-Item C:\Windows\temp\ShareX\conf\*  C:\ProgramData\ShareX\ -Force
